...
export default class Rank extends React.Component {
  ...
  render() {
    ...
    (data.ele_yesterday || []).forEach((item, index) => (item.key = index));
    ...
    if (this.state.rank === 'ele_yesterday') {
      columns.push({title: '今日赠送美团次数', dataIndex: 'meituan'});
    ...
    return (
      <div>
        ...
        <div>【近期上线】饿了么昨日排名前N，今日赠送对应的美团次数</div>
        <div style={{marginTop: 15, paddingBottom: 15}}>
          ...
          <Radio.Group onChange={this.onChange} value={this.state.rank}>
            ...
            <Radio value="ele_yesterday">饿了么昨日</Radio>
          </Radio.Group>
        </div>
